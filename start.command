...
api_is_current() {
  /usr/bin/curl --silent --fail --connect-timeout 1 --max-time 3 "$API_HEALTH_URL" 2>/dev/null | /usr/bin/grep -q '"apiVersion":11'
}

web_is_current() {
  /usr/bin/curl --silent --fail --connect-timeout 1 --max-time 5 "$1/strategy" 2>/dev/null | /usr/bin/grep -q 'data-fund-flow-version="10"'
}
